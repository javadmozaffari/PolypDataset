...
def train_one_epoch(loader, model, criterion, optimizer, writer, epoch, lr_scheduler, model_type, logger):
    ...
    for i, (sampled_batch, names) in enumerate(loader, 1):
        ...
        preds, preds2 = model(images, text)
        out_loss = criterion(preds, masks.float())+criterion(preds2, masks.float()) 
        preds2 = preds+ preds2
        if model.training:
            optimizer.zero_grad()
            out_loss.backward()
        ...
        train_dice = criterion._show_dice(preds2, masks.float())
        train_iou = iou_on_batch(masks,preds2)
        ...
        if epoch % config.vis_frequency == 0 and logging_mode is 'Val':
            ...
            save_on_batch(images,masks,preds2,names,vis_path)
