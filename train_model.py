...
from nets.VLM import VLM
from nets.ConTEXTual_model import ConTEXTual_model
...
def save_checkpoint(state, save_path):
    ...
    if best_model:
        ...
        filename = save_path + '/' +'last_model.pth.tar'
...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    if config.task_name == 'Polyp':
        train_text = read_text(config.train_dataset + 'Train_text.xlsx')
        # train_text = read_text(config.val_dataset + 'Val_text.xlsx')
        ...
        # train_dataset = ImageToImage2D(config.val_dataset, config.task_name, val_text, val_tf, image_size=config.img_size)
    ...
    if model_type == 'Train':
        ...
        model = VLM(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)
        ...
        logger.info('transformer expand ratio: {}'.format(config_vit.expand_ratio))
        model = VLM(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)
        pretrained_UNet_model_path = "Polyp/Model/best_model.pth.tar"
